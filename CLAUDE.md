...
| **A CHECK THAT CANNOT FAIL** | **a check** | The class, not four instances of it. A masked failure reported as success, four times in one night: `cmd \| tail` returns `tail`'s status, so a **segfaulting** export (exit 139) read as `exit 0` — then the same mask inside the diagnostic written to find it; a freshness guard that read `$(git rev-parse origin/main)` *after* `git fetch`, comparing the post-fetch value with itself, inside the very protocol step it protected; and a stale mtime read as proof a job stopped early, when `_canonical()` strips `generated_at` so a part that never ran is byte-identical to one that ran unchanged. **The test is not "did it pass" but "could it have failed".** Before trusting any check, make it fail on purpose once. Track F states the same rule from the other side: *a guard that has never been seen to fail is not a guard* |
